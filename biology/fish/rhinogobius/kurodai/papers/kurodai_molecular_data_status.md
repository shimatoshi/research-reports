...
R. kurodai はタイプ産地の消滅、タイプ標本の損壊、およびタイプ標本からのDNA抽出不能により、現存集団への名前の適用に分子的根拠を欠く。再記載（Suzuki & Chen 2011）は損壊したホロタイプの限られた形態特徴に基づくものであり、種の実在性や現存集団との遺伝的連続性を証明するものではない。

本プロジェクトでは **R. kurodai を nomen dubium（疑問名）として扱い**、「kurodai」の名前に依存しない集団ベースの分析を行う。ただし、既存の配列データ（Yamasaki et al. 2015 等）は「東京・新宿御苑の Rhinogobius sp. OR 偽橙色型」の配列として有用であり、集団レベルの分析に活用する。
